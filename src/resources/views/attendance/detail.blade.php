        ...
        <table class="detail-table">
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-black align-left">
                        @php
                            $clockInValue = $attendance->status==='pending' && $pendingPayload
                                ? (isset($pendingPayload->after_attendance['clock_in_at']) ? \Carbon\Carbon::parse($pendingPayload->after_attendance['clock_in_at'])->format('H:i') : '')
                                : old('after_attendance.clock_in_at', $attendance->clock_in_at ? \Carbon\Carbon::parse($attendance->clock_in_at)->format('H:i') : '');
                            $clockOutValue = $attendance->status==='pending' && $pendingPayload
                                ? (isset($pendingPayload->after_attendance['clock_out_at']) ? \Carbon\Carbon::parse($pendingPayload->after_attendance['clock_out_at'])->format('H:i') : '')
                                : old('after_attendance.clock_out_at', $attendance->clock_out_at ? \Carbon\Carbon::parse($attendance->clock_out_at)->format('H:i') : '');
                            $readonly = $attendance->status==='pending' ? 'readonly tabindex="-1"' : '';
                            $inputClass = 'detail-input text-black align-input' . ($attendance->status==='pending' ? ' input-readonly' : '');
                        @endphp
                ...
                @php
                    $breakStartValue = $attendance->status==='pending' && $pendingPayload
                        ? (isset($pendingPayload->after_breaks[$i]['start_at']) ? \Carbon\Carbon::parse($pendingPayload->after_breaks[$i]['start_at'])->format('H:i') : '')
                        : old('after_breaks.'.$i.'.start_at', $break->start_at ? \Carbon\Carbon::parse($break->start_at)->format('H:i') : '');
                    $breakEndValue = $attendance->status==='pending' && $pendingPayload
                        ? (isset($pendingPayload->after_breaks[$i]['end_at']) ? \Carbon\Carbon::parse($pendingPayload->after_breaks[$i]['end_at'])->format('H:i') : '')
                        : old('after_breaks.'.$i.'.end_at', $break->end_at ? \Carbon\Carbon::parse($break->end_at)->format('H:i') : '');
                @endphp
                <tr>
                ...
                <tr>
                    ...
                    <td class="text-black align-left">
                        <div>
                        <textarea name="after_attendance[note]" class="detail-note-input text-black align-input @if($attendance->status==='pending') input-readonly @endif" rows="5" @if($attendance->status==='pending') readonly tabindex="-1" @endif>{{ $attendance->status==='pending' && $pendingPayload ? ($pendingPayload->after_attendance['note'] ?? '') : old('after_attendance.note', $attendance->note) }}</textarea>
                        </div>
